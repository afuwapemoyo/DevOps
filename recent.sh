...
then
  ...
  JPATH=$(echo $REPO_PATH | sed "s/$REPO\///g" | sed -e 's/\/$//')
  echo "Repo : $REPO and Path : $JPATH"
  query='items.find({"stat.downloads":{"$eq":null},"created" : {"$before" : "60s"},"repo":{"$eq":"'$REPO'"},"path":{"$match":"*'$JPATH'*"}}).include("stat.downloaded")'
  echo ""
  echo "checking $REPO on path : $JPATH" ...
else
  query='items.find({"stat.downloads":{"$eq":null},"created" : {"$before" : "60s"},"repo":{"$eq":"'$REPOSITORY'"}}).include("stat.downloaded")'
  echo ""
...
for row in $(echo "${result}" | jq -r '.results[] | @base64'); do
  ...
  if [ $dry_run = true ]
    then
      ...
      echo "$(_jq '.name') | $URL/"$(_jq '.repo')"/"$(_jq '.path')"/"$(_jq '.name') >> artifact-list
    else
      #real file deletion
