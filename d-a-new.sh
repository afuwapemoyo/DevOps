...
echo "Getting artifacts : never download since 60 days  ..."
echo ""
query='items.find({"stat.downloaded":{"$before":"60d"},"repo":{"$eq":"'$REPOSITORY'"}}).include("stat.downloaded")'
#set AQL query, in this case we want to retrieve artifact on some repo that its not has been downloaded since 60 days 
query='items.find({"stat.downloads":{"$eq":null},"created" : {"$before" : "60s"},"repo":{"$eq":"'$REPOSITORY'"}})'
...
if [ $dry_run = true ]
  then
    echo "ARTIFACT | URL" > artifact-list
